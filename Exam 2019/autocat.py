...
class Autocat:
    ...
    def update(self, k):
        ...
        for z in range(self.sweeps_per_update):
            ...
            u_update = self.dt* (np.multiply(self.D1,(self.u_grid+self.lap2D(self.u_grid))))\
                    - self.dt * np.multiply(self.u_grid, np.square(self.v_grid))\
                     + self.dt * self.F * (1 - self.u_grid)


            v_update = self.dt*(np.multiply(self.D2,(self.v_grid+self.lap2D(self.v_grid))))\
             + self.dt * np.multiply(self.u_grid, np.square(self.v_grid))\
        ...
        for z in range(self.sweeps_per_update):
            ...
            for i in range(self.N):
                for j in range(self.N):
                    ...
                    deltaU = (self.D1*self.dt) * (self.u_grid[i][j] + self.laplacian_(self.u_grid, i, j))\
                            - self.dt * self.u_grid[i][j]*self.v_grid[i][j]**2 \
                            + self.dt * self.F*(1-self.u_grid[i][j])
                    new_u_grid[i][j] += deltaU
                    deltaV = (self.D2*self.dt) * (self.v_grid[i][j] + self.laplacian_(self.v_grid, i, j))\
                            + self.dt*self.u_grid[i][j]*self.v_grid[i][j]**2 \
